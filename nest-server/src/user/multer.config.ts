...
const storage = multer.diskStorage({
  ...
  filename: function (req, file, cb) {
    ...
    file['__uploadPath'] = '/media/user/' + fname;
...
function fileFilter(req: Request, file: Express.Multer.File, cb: any) {
  const allowedMimeTypes = ['image/jpeg', 'image/jpeg'];
  const allowedFilesExtension = ['.png', '.jpg', '.jpeg' ];
...
export const MulterConfigOptions: multer.Options = {
  limits: {
    fileSize: 1024 * 1024 * 1, // each file should less then 1 MB
  },
